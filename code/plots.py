...
def plot_histogram(master, output_dir):
    ...
    bin_edges = np.arange(0, 2601, 100)
...
.hover-zone:hover{fill:rgba(100,181,246,.28)}.axis-label{fill:#222;font-size:16px}.axis-title{fill:#222;font-size:14px}
...
const width=600,height=440,margin={top:20,right:15,bottom:85,left:75};
...
const xTicks=[0,200,400,600,800,1000,1200,1400,1600,1800,2000,2200,2400,2600];
xTicks.forEach(value=>{const lineX=x(value);svg.appendChild(element('line',{x1:lineX,y1:margin.top,x2:lineX,y2:height-margin.bottom,class:'grid'}));const label=element('text',{x:lineX,y:height-margin.bottom+25,'text-anchor':'end',transform:`rotate(-45 ${lineX} ${height-margin.bottom+25})`,class:'axis-label'});label.textContent=value.toLocaleString('cs-CZ');svg.appendChild(label)});
...
const xTitle=element('text',{x:margin.left+plotWidth/2,y:height-10,'text-anchor':'middle',class:'axis-title'});xTitle.textContent='STR';svg.appendChild(xTitle);
